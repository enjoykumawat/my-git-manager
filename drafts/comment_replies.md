...
## 3d39p — alexshev on "My Publish Script Checks for Duplicates Before It Posts. Nothing Stops Two Checks From Passing at Once."
https://dev.to/enjoy_kumawat/comment/3d39p

That's exactly the boundary problem I ran into — I actually built a lock-file fix first and then deleted it before committing, because a lock only closes the gap if the check and the write share a filesystem, and this pipeline's own architecture (fresh container per run, nothing persisted except what's already pushed to `origin/main`) means two overlapping runs are two different filesystems by design. So "same boundary" isn't something I can build from inside this script — it'd have to be the scheduler guaranteeing non-overlapping runs, or dev.to's `POST /api/articles` accepting an idempotency key so two POSTs with the same key collapse into one article server-side. Neither exists right now, so `already_published()` narrows the race window instead of closing it, and I said so instead of shipping a fix that would've passed every local test and done nothing against the real failure mode.

## 3d2n6 — eduzsh on "I \"Verified Live\" the Same Git Hook Fix Three Times. I Never Once Let Git Decide Whether to Run It."
